...
def get_PEheaders_features(path):

    PEheaders_features = []
    pe = pefile.PE(path)
    if hasattr(pe, 'OPTIONAL_HEADER'):
        header = pe.OPTIONAL_HEADER
        for it in header.__keys__:
            peheader_features += [header.__getattribute__(it[0])]
    print(PEheaders_features)
    print("Extracted {0} PE Headers from {1}".format(len(peheader_features), path))
    return PEheaders_features
...
def scan_file(path):
    ...
    features = get_PEheaders_features(path)
...
def train_detector(benign_path,malicious_path):
    ...
    X = [get_PEheaders_features(path) for path in malicious_paths + benign_paths]
